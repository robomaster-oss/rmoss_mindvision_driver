...
: camera_sn_(camera_sn), node_(node), config_path_(config_path)
{
  ...
  _param[rmoss_cam::CamParamType::Contrast] = 0;
  _param[rmoss_cam::CamParamType::Saturation] = 0;
  _param[rmoss_cam::CamParamType::Hue] = 0;
...
bool MindVisionCam::open()
{
  ...
  if (config_path_ != "") {
    status = CameraReadParameterFromFile(hCamera_, const_cast<char *>(config_path_.c_str()));
    if (status != CAMERA_STATUS_SUCCESS) {
      RCLCPP_WARN(
        node_->get_logger(),
...
{
  if (CameraSaveParameterToFile(hCamera_, const_cast<char *>(path.c_str())) !=
